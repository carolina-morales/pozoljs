...
export class ServiceSchematic extends AbstractSchematic {
	...
	public async handle() {
		...
		lists.push(generateTask('Creating service file...', () => this.createFiles()));
	...
	private createFiles(): void {
		...
		createFile(this.destPath, language, 'service', this.schematicName);
	}

	private async askIfOverrideFile(): Promise<boolean> {
		const language = this.inputs.find((input) => input.name === 'language')!.value.toString().toLowerCase();

		return await askIfOverrideFile(this.destPath, language, this.schematicName, 'service');
	}
}
